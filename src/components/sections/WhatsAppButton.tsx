...
export default function WhatsAppButton({ number, productName }: Props) {
  ...
  return (
    <a
      ...
      className="fixed bottom-5 right-4 z-50 flex items-center gap-2.5 rounded-full bg-[#25D366] text-white shadow-2xl shadow-green-900/40 ring-4 ring-white pl-4 pr-6 py-4 hover:scale-105 active:scale-95 transition-transform"
    >
      {/* Pulsing glow ring */}
      <span
        aria-hidden
      ...
      <span className="text-base font-extrabold relative whitespace-nowrap">
        Chat with us
      </span>
    </a>
  );
